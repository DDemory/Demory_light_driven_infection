function out=mcmcpred_modif(results,chain,s2chain,data,modelfun,nsample,varargin)
...
for i=1:nbatch
  ...
  for iisample = 1:nsample;
    ...
    yres   = feval(modelfun,datai,th,varargin{:});
    
    % Calculate for Htot and V
    y = [yres(:,1)+yres(:,2)+yres(:,3),yres(:,4)];
    ysave(iisample,:,:) = y;
